...
void SOR(double ***P, double ***B, double ***Ap, double ***Ae, double ***Aw, double ***An, double ***As, double ***Af, double ***Ab, double X, double Y, double Z, double h, int nx, int ny, int nz, double w, int itr, int sit, char boundaryType)
{
     ...
     SET_BOUNDARY(P, X, Y, Z, h, nx, ny, nz, boundaryType, sit);
     ...
     for(it=1; it<=itr; it++)
     {
  	      for(i=1; i<(nx-1); i++)
               for(j=1; j<(ny-1); j++)
			        for(k=1; k<(nz-1); k++)
                         P[i][j][k] = (w/Ap[i][j][k])*(B[i][j][k]-(Ae[i][j][k]*P[i+1][j][k]+
                                      Aw[i][j][k]*P[i-1][j][k]+An[i][j][k]*P[i][j+1][k]+
                                      As[i][j][k]*P[i][j-1][k]+Af[i][j][k]*P[i][j][k+1]+
                                      Ab[i][j][k]*P[i][j][k-1]))+(1-w)*P[i][j][k];
...
{     
     ...
     {             
		  ...
		  SOR(P, B, Ap[t], Ae[t], Aw[t], An[t], As[t], Af[t], Ab[t], X, Y, Z, h[t], nx[t], ny[t], nz[t], w[t], downTimes[t], 1, boundaryType);

          RESIDUE(R[t], P, B, Ap[t], Ae[t], Aw[t], An[t], As[t], Af[t], Ab[t], nx[t], ny[t], nz[t]);
 
          RSD = MAXIMUM(R[t], nx[t], ny[t], nz[t]);
          
          cout << "NC = " << NC << "     :     RESIDUE = " << RSD << endl;           
          fprintf(p1, "%i       %e\n", NC, RSD);             
          EMAX = INFINITY_NORM(Pex, P, E, nx[t], ny[t], nz[t]);	      	      
          fprintf(p2, "%i       %e\n", NC, EMAX); 
                
          while(t > 0)
          {
               RESTRICTION(R[t], rhs, nx, ny, nz, t);
               t--;               	
               SOR(e[t], rhs[t], Ap[t], Ae[t], Aw[t], An[t], As[t], Af[t], Ab[t], X, Y, Z, h[t], nx[t], ny[t], nz[t], w[t], downTimes[t], 0, boundaryType);
               RESIDUE(R[t], e[t], rhs[t], Ap[t], Ae[t], Aw[t], An[t], As[t], Af[t], Ab[t], nx[t], ny[t], nz[t]);               
          }

          while(t < (levels-1))
          {
               INTERPOLATION(e, nx, ny, nz, t);
               t++;			
               if(t != (levels-1))
                    SOR(e[t], rhs[t], Ap[t], Ae[t], Aw[t], An[t], As[t], Af[t], Ab[t], X, Y, Z, h[t], nx[t], ny[t], nz[t], w[t], upTimes[t], 0, boundaryType);              			
               else
               {                     
                    CORRECT(P, e[t], nx[t], ny[t], nz[t]);
                    ZERO(e, nx, ny, nz, levels);				     
               }
          }                                  
     }
     while(RSD > eps);
...
void CONJUGATE_GRADIENT(double ***Pex, double ***P, double ***B, double ***E, double ***z1, double ***z2, double ***R1, double ***R2, double ***p, double ***W, double ****Ap, double ****Ae, double ****Aw, double ****An, double ****As, double ****Af, double ****Ab, double X, double Y, double Z, double *h, int *nx, int *ny, int *nz, double *w, double eps, int levels, char boundaryType, char preconditionType)
{
     ...
     SET_BOUNDARY(P, X, Y, Z, h[t], nx[t], ny[t], nz[t], boundaryType, 1);
     RESIDUE(R1, P, B, Ap[t], Ae[t], Aw[t], An[t], As[t], Af[t], Ab[t], nx[t], ny[t], nz[t]);
     rsd = MAXIMUM(R1, nx[t], ny[t], nz[t]);
     emax = INFINITY_NORM(Pex, P, E, nx[t], ny[t], nz[t]);
     ...
     if(preconditionType == 's')    
          EQUAL(z1, R1, nx[t], ny[t], nz[t]); 
     else if(preconditionType == 'j')
          JACOBI(z1, R1, Ap[t], nx[t], ny[t], nz[t]);   
      
      
     EQUAL(p, z1, nx[t], ny[t], nz[t]);
     SET_BOUNDARY(p, X, Y, Z, h[t], nx[t], ny[t], nz[t], boundaryType, 0);      
          ...
          for(j=1; j<(ny[t]-1); j++)
               for(k=1; k<(nz[t]-1); k++)
                    W[i][j][k] = Ap[t][i][j][k]*p[i][j][k] + Ae[t][i][j][k]*p[i+1][j][k] + Aw[t][i][j][k]*p[i-1][j][k] 
                                 ...
                                 + Af[t][i][j][k]*p[i][j][k+1] + Ab[t][i][j][k]*p[i][j][k-1];
                     
     alfa = INTERNAL_PRODUCT(R1, z1, nx[t], ny[t], nz[t])/INTERNAL_PRODUCT(p, W, nx[t], ny[t], nz[t]);
     ...
     do
     {
          ...
          SET_BOUNDARY(P, X, Y, Z, h[t], nx[t], ny[t], nz[t], boundaryType, 1);
          RESIDUE(R2, P, B, Ap[t], Ae[t], Aw[t], An[t], As[t], Af[t], Ab[t], nx[t], ny[t], nz[t]);                
          rsd = MAXIMUM(R2, nx[t], ny[t], nz[t]);
          emax = INFINITY_NORM(Pex, P, E, nx[t], ny[t], nz[t]);
          ...
          if(preconditionType == 's')    
               EQUAL(z2, R2, nx[t], ny[t], nz[t]); 
          else if(preconditionType == 'j')
               JACOBI(z2, R2, Ap[t], nx[t], ny[t], nz[t]);   
      
          
          beta = INTERNAL_PRODUCT(R2, z2, nx[t], ny[t], nz[t])/INTERNAL_PRODUCT(R1, z1, nx[t], ny[t], nz[t]);
          ...
          alfa = INTERNAL_PRODUCT(R2, z2, nx[t], ny[t], nz[t])/INTERNAL_PRODUCT(p, W, nx[t], ny[t], nz[t]);

          EQUAL(R1, R2, nx[t], ny[t], nz[t]);
          EQUAL(z1, z2, nx[t], ny[t], nz[t]);           
     }
     while(rsd > eps);
...
void nested(double ****Pex, double ****E, double ****e, double ****R, double ****rhs, double ****z1, double ****z2, double ****R1, double ****R2, double ****p, double ****W, double ****Ap, double ****Ae, double ****Aw, double ****As, double ****An, double ****Au, double ****Ad, double ****Mp, double ****Me, double ****Mw, double ****Ms, double ****Mn, double ****Mu, double ****Md, double ****B, double ****P, int *nx, int *ny, int *nz, double *h, int niveis, int fun, char fron, double L, double D, double S, int *itr_u, int *itr_d, double eps, double *w)
{
      ...
      for(t=0; t<niveis; t++)
      {
           ...
           cg(Pex[t], P[t], B[t], E[t], z1[t], z2[t], R1[t], R2[t], p[t], W[t], R, rhs, e, Ap, Ae, Aw, An, As, Au, Ad, Ap, Ap, Ap, Ap, Ap, Ap, Ap, Ap, nx, ny, nz, D, L, S, h, w, itr_d, itr_u, eps, fun, fron, 'j', NIVEIS);
